...
(function (Guidebook) {
    ...
    const guideBookQuery = { type: "spark.portals:guide_book" };
    const bookSpawnActionBarRange = 8;
    let bookItemId, projectLangName;
    let disasterControllerID = "spark_disasters:disaster_controller";
    ...
    function checkForSpawn(book) {
        if (book.hasTag(`spark_${projectLangName}:action_bar_sent`))
            return;
        ...
        for (const player of book.dimension.getPlayers({ location: book.location, maxDistance: bookSpawnActionBarRange })) {
            player.onScreenDisplay.setActionBar({
                rawtext: [
                    {
                        translate: `spark_${projectLangName}.book.open`
                    }
                ]
            });
        }
        book.addTag(`spark_${projectLangName}:action_bar_sent`);
    }
    ...
    world.afterEvents.playerSpawn.subscribe((event) => {
        if (event.initialSpawn) {
            ...
            if (!entityHasItem(event.player, bookItemId))
                event.player.runCommandAsync(`give @s ${bookItemId}`);
            if (!entityHasItem(event.player, disasterControllerID))
                event.player.runCommandAsync(`give @s ${disasterControllerID}`);
            ...
            system.runTimeout(() => {
                event.player.sendMessage({
                    rawtext: [
                        {
                            translate: `spark_${projectLangName}.chat.is_active`
                        }
                    ]
                });
            }, 100);
            system.runTimeout(() => {
                event.player.sendMessage({
                    rawtext: [
                        {
                            translate: `spark_${projectLangName}.chat.info_message`
                        }
                    ]
                });
            }, 200);
    ...
    function entityHasItem(entity, item) {
